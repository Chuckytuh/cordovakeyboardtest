...
The page also has a status bar toggle button and a bare text input with no attached JS (its purpose isn't documented in code). The toggle button reads/writes `window.statusbar.visible`, and what that actually does differs by cordova-android version — this is itself a good example of the kind of cross-version behavior difference this workspace exists to surface:
- On **cordova-android 14** (`cordova-android14`, `cordova-android14_statusbar`), `window.statusbar` is the unmodified, standard, read-only legacy browser `BarProp` API (always `{ visible: true }` for a non-popup window) — assigning to `.visible` silently fails, so the button never affects the real status bar in either project, even where `cordova-plugin-statusbar` is installed (that plugin only exposes `window.StatusBar`, which this code never references).
- On **cordova-android 15** (unreleased as of this writing; `master` is `15.1.0-dev.0`), core now ships a built-in `SystemBarPlugin` whose JS shim clobbers `window.statusbar` with a writable object (`cordova-js-src/plugin/android/statusbar.js`): the setter forwards to `window.StatusBar.show()/hide()` if `cordova-plugin-statusbar` is present, otherwise calls native `exec(..., 'SystemBarPlugin', 'setStatusBarVisible', ...)`. Natively, `SystemBarPlugin.execute()` no-ops immediately whenever the `AndroidEdgeToEdge` preference is `true`. None of our cordova-android15 projects has `cordova-plugin-statusbar` installed, so in practice: the button is a no-op in `cordova-android15_e2e`/`_e2e_insetinjector` (`AndroidEdgeToEdge=true`, native call short-circuits), but genuinely hides/shows the status bar in `cordova-android15_no_e2e`/`_no_e2e_insetinjector` (`AndroidEdgeToEdge=false`).
